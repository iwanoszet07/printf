...
int loop_format(va_list arg, const char *string)
{
	...
	while (i < _strlen((char *)string) && *string != '\0')
	{
		...
		if (aux == '%')
		{
			...
			{
				cont_fm = function_builder(aux, arg);
...
/**
 * check_percent - call function builder
...
/**
 * call_function_builder - call function builder
 *@aux: character parameter
 *@arg: va_list arg
 *Description: This function call function builder
 *Return: num of characteres printed
 */

int call_function_builder(char aux, va_list arg)
{
	int cont = 0;

	cont = function_builder(aux, arg);
	return (cont);
}
